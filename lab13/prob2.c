...
void *thread(void *vargp){
	...
	int seed;
	int id;
	long *gentime, *cmptime;

	printf("is there?\n");
	struct my_data temp = *((struct my_data *)vargp);
	printf("tid: %d, gentime: %p, cmp: %p\n", id, gentime, cmptime);
	id		= temp.it;
	gentime = temp.gentime;
	cmptime = temp.cmptime;
	printf("tid: %d, gentime: %p, cmp: %p\n", id, gentime, cmptime);
	...
	seed = i; /* it must be a thread id (0, 1, .. N-1 if there are N threads) */
	srand (seed);
...
double run_test (long num_threads, long* gentime, long* cmptime)
{
	...
	for (i = 0; i < num_threads; i++){
		temp[i].it = i;
		temp[i].gentime = gentime;
		temp[i].cmptime = cmptime;
		printf("temp: (%d, %p, %p)\n", temp[i].it, temp[i].gentime, temp[i].cmptime);
		pthread_create(&tid[i], NULL, tester, (void *)&temp[i]);
		printf("creating %d thread: %d\n", i, tid[i]);
	}
	printf("is there2?\n");
	for (i = 0; i < num_threads; i++){
		double *ret;
